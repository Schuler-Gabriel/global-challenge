    ...
    for variable, (min_val, max_val) in VALID_RANGES.items():
        ...
        if column_name in df.columns:
            ...
            validation_results[variable] = {
                ...
                'out_of_range_percentage': (out_of_range / non_null_values * 100) if non_null_values > 0 else 0,
...
def analyze_data_coverage() -> Dict:
    ...
    station_summary = inventory_df.groupby('station_type').agg({
        'filename': 'count',
        'file_size': 'sum',
        'start_date': 'min',
        'end_date': 'max'
    ...
    station_summary.columns = ['Num_Arquivos', 'Tamanho_Total_KB', 'Data_Inicial', 'Data_Final']
...
def sample_data_analysis():
    ...
    a801_old_files = [f for f in csv_files if 'A801' in f and 'JARDIM BOTANICO' not in f]
    if a801_old_files:
    ...
    for key, filepath in sample_files.items():
        ...
        if not df.empty:
            ...
            analysis_results[key] = {
                ...
                'missing_data_percentage': (missing_data / (total_records * len(df.columns)) * 100),
                'validation': validation
            }
            
            print(f"      ✅ {total_records:,} registros ({date_range[0]} a {date_range[1]})")
            print(f"         📊 Dados faltantes: {missing_data:,} ({missing_data / (total_records * len(df.columns)) * 100:.1f}%)")
        else:
...
def station_comparison_analysis(sample_data: Dict) -> Dict:
    ...
    main_variables = ['precipitation', 'temperature', 'humidity', 'pressure', 'wind_speed']
    ...
    for variable, stats in comparison_results.items():
        ...
        for station, values in stats.items():
            print(f"   {station}: μ={values['mean']:.2f}, σ={values['std']:.2f}, n={values['count']:,}")
...
def generate_visualizations(sample_data: Dict):
    ...
    for i, (key, df) in enumerate(sample_data.items()):
        ...
        plt.barh(y_pos, (end_date - start_date).days, 
                left=start_date, height=0.6, 
                color=color, alpha=0.7, label=key)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_PATH, 'temporal_coverage.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if precipitation_data:
        precip_df = pd.DataFrame(precipitation_data, columns=['Station', 'Precipitation'])
    ...
    for key, df in sample_data.items():
        ...
        if temp_col in df.columns and humid_col in df.columns:
            ...
            if len(common_idx) > 0:
                plt.scatter(temp_sample.loc[common_idx], humid_sample.loc[common_idx], 
                           alpha=0.6, label=key, s=10)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_PATH, 'meteorological_analysis.png'), dpi=300, bbox_inches='tight')
    plt.close()
