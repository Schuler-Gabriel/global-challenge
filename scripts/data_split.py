...
class TemporalDataSplitter:
    ...
    def load_consolidated_data(self, data_dir: str) -> pd.DataFrame:
        ...
        logger.info(f"Dados consolidados: {len(consolidated_df)} registros de {consolidated_df.index.min()} até {consolidated_df.index.max()}")
    ...
    def add_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_with_features['season'] = df_with_features['month'].map({
            12: 'Summer', 1: 'Summer', 2: 'Summer',  # Verão (Hemisfério Sul)
            3: 'Autumn', 4: 'Autumn', 5: 'Autumn',   # Outono
            6: 'Winter', 7: 'Winter', 8: 'Winter',   # Inverno
        ...
        df_with_features['day_year_sin'] = np.sin(2 * np.pi * df_with_features['day_of_year'] / 365.25)
        df_with_features['day_year_cos'] = np.cos(2 * np.pi * df_with_features['day_of_year'] / 365.25)
        
        logger.info(f"Features temporais adicionadas: {df_with_features.shape[1]} colunas totais")
        return df_with_features
    
    def calculate_split_statistics(self, df: pd.DataFrame, split_name: str) -> SplitStatistics:
        """
    ...
    def temporal_split_by_years(self, df: pd.DataFrame, 
        ...
        logger.info(f"  Treino: {train_year_list[0]}-{train_year_list[-1]} ({len(train_df)} registros)")
        logger.info(f"  Validação: {val_year_list[0]}-{val_year_list[-1]} ({len(val_df)} registros)")
        logger.info(f"  Teste: {test_year_list[0]}-{test_year_list[-1]} ({len(test_df)} registros)")
        
        return train_df, val_df, test_df
    
    def stratified_decade_split(self, df: pd.DataFrame,
                               train_ratio: float = 0.7,
                               val_ratio: float = 0.15,
                               test_ratio: float = 0.15) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        for decade in sorted(df['decade'].unique()):
            ...
            val_years = decade_years[train_years_count:train_years_count + val_years_count]
        ...
        # Consolida splits
        train_df = pd.concat(train_dfs, axis=0).sort_index() if train_dfs else pd.DataFrame()
        val_df = pd.concat(val_dfs, axis=0).sort_index() if val_dfs else pd.DataFrame()
        test_df = pd.concat(test_dfs, axis=0).sort_index() if test_dfs else pd.DataFrame()
    ...
    def walk_forward_validation_splits(self, df: pd.DataFrame, 
        ...
        for fold, (train_idx, val_idx) in enumerate(tscv.split(df_array)):
            ...
            logger.info(f"Walk-forward fold {fold + 1}: "
                       f"train {len(train_data)} registros, "
                       f"val {len(val_data)} registros")
        
        return splits
    
    def analyze_seasonal_balance(self, train_df: pd.DataFrame, 
                               val_df: pd.DataFrame, 
    ...
    def analyze_decade_distribution(self, train_df: pd.DataFrame,
                                  val_df: pd.DataFrame,
                                  test_df: pd.DataFrame) -> Dict[str, Dict[str, int]]:
        """
    ...
    def save_splits(self, train_df: pd.DataFrame,
        ...
        for split_name, split_df in [('train', train_df), ('validation', val_df), ('test', test_df)]:
            if not split_df.empty:
                ...
                logger.info(f"Split {split_name} salvo: {filepath} ({len(split_df)} registros)")
        
        return output_files
    
    def create_splits(self, data_dir: str, 
                     strategy: str = "temporal_years",
                     **kwargs) -> DataSplitReport:
        """
        ...
        Returns:
            Relatório do split
        """
        logger.info(f"=== INICIANDO SPLIT DE DADOS - ESTRATÉGIA: {strategy.upper()} ===")
        ...
        if df.empty:
            ...
            return DataSplitReport(
                ...
                validation_stats=SplitStatistics("validation", "", "", 0, 0.0, [], {}, 0.0),
        ...
        decade_distribution = self.analyze_decade_distribution(train_df, val_df, test_df)
    ...
    def print_split_summary(self, report: DataSplitReport) -> None:
        ...
        for stats in [report.train_stats, report.validation_stats, report.test_stats]:
            ...
            print(f"     Anos: {len(stats.years_covered)} ({min(stats.years_covered) if stats.years_covered else 'N/A'}-{max(stats.years_covered) if stats.years_covered else 'N/A'})")
            print(f"     Dados faltantes: {stats.missing_data_percentage:.1f}%")
        
        print(f"\n🌱 BALANÇO SAZONAL:")
        for split_name, balance in report.seasonal_balance.items():
            print(f"   {split_name.capitalize()}: {balance:.3f} (0=desbalanceado, 1=perfeito)")
        
        print(f"\n📅 DISTRIBUIÇÃO POR DÉCADA:")
        for split_name, decade_dist in report.decade_distribution.items():
            if decade_dist:
                decades_str = ", ".join([f"{decade}s: {count}" for decade, count in decade_dist.items()])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Split temporal de dados meteorológicos INMET")
    parser.add_argument("data_dir", help="Diretório com arquivos CSV de dados")
    parser.add_argument("--strategy", choices=["temporal_years", "stratified_decade"], 
                       default="temporal_years", help="Estratégia de split")
