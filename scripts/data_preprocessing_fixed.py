...
class DataPreprocessorFixed:
    ...
    def standardize_datetime(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_clean.columns:
            col_clean = col.strip()
            if 'DATA' in col_clean.upper() and ('YYYY' in col_clean.upper() or col_clean == 'Data'):
                date_col = col
            elif 'HORA' in col_clean.upper() and ('UTC' in col_clean.upper() or col_clean == 'Hora UTC'):
                time_col = col
        ...
        if date_col and time_col:
            ...
            try:
                ...
                time_normalized = time_values.apply(lambda x: f"{x[:2]}:{x[2:]}" if len(x) == 4 and x.isdigit() else x)
                ...
                logger.info(f"✅ Timestamps criados: {success_count}/{len(df_clean)} ({success_rate:.1f}%)")
                
            except Exception as e:
                logger.warning(f"⚠️ Erro ao criar timestamps: {str(e)}")
                # Fallback: usar apenas data como timestamp
                df_clean['timestamp'] = pd.to_datetime(df_clean[date_col], errors='coerce')
                # Garantir que seja timezone-naive
                if df_clean['timestamp'].dt.tz is not None:
                    df_clean['timestamp'] = df_clean['timestamp'].dt.tz_localize(None)
        else:
            logger.warning(f"⚠️ Colunas de data/hora não encontradas. Procurando por 'DATA' e 'HORA'")
    ...
    def standardize_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        missing_codes = ['-9999', '-999', '999999', 'null', 'NULL', '', ' ', '---', '--', '-']
        for col in df_clean.columns:
            if col not in ['timestamp', 'Data', 'Hora UTC', 'DATA (YYYY-MM-DD)', 'HORA (UTC)']:
    ...
    def handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_columns:
            ...
            if isinstance(missing_count, (pd.Series, np.ndarray)):
                count_val = missing_count.iloc[0] if isinstance(missing_count, pd.Series) else missing_count.item()
            else:
                count_val = int(missing_count)
            
            if isinstance(missing_pct, (pd.Series, np.ndarray)):
                pct_val = missing_pct.iloc[0] if isinstance(missing_pct, pd.Series) else missing_pct.item()
            else:
        ...
        for col in numeric_columns:
            ...
            if missing_pct > 80:
                ...
                logger.warning(f"⚠️ {col} tem {missing_pct:.1f}% missing - mantendo para análise")
                continue
            ...
            if 'precipitacao' in col.lower() or 'precipita' in col.lower():
                ...
                try:
                    ...
                    df_clean[col] = df_clean[col].interpolate(
                        method='time', 
                        limit=self.config.max_gap_hours
                    )
            ...
            if isinstance(remaining_missing, (pd.Series, np.ndarray)):
                remaining_count = remaining_missing.iloc[0] if isinstance(remaining_missing, pd.Series) else remaining_missing.item()
            else:
    ...
    def create_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_columns = ['temperatura_c', 'umidade_relativa', 'precipitacao_mm', 
                          'pressao_hpa', 'vento_velocidade_ms', 'radiacao_kjm2',
                          'pressao_mb', 'vento_direcao_graus', 'temperatura_ponto_orvalho_c',
                          'temp_max_c', 'temp_min_c', 'umidade_max', 'umidade_min']
        ...
        has_temperature = 'temperatura_c' in df_enhanced.columns and df_enhanced['temperatura_c'].notna().any()
        has_pressure = 'pressao_mb' in df_enhanced.columns and df_enhanced['pressao_mb'].notna().any()
        has_humidity = 'umidade_relativa' in df_enhanced.columns and df_enhanced['umidade_relativa'].notna().any()
        has_wind = 'vento_velocidade_ms' in df_enhanced.columns and df_enhanced['vento_velocidade_ms'].notna().any()
        has_precipitation = 'precipitacao_mm' in df_enhanced.columns and df_enhanced['precipitacao_mm'].notna().any()
        ...
        if 'temperatura_c' in df_enhanced.columns and df_enhanced['temperatura_c'].notna().any():
        ...
        if 'vento_velocidade_ms' in df_enhanced.columns and 'vento_direcao_graus' in df_enhanced.columns:
            ...
            df_enhanced['vento_u'] = -df_enhanced['vento_velocidade_ms'] * np.sin(wind_rad)
            df_enhanced['vento_v'] = -df_enhanced['vento_velocidade_ms'] * np.cos(wind_rad)
        ...
        if 'precipitacao_mm' in df_enhanced.columns:
            ...
            df_enhanced['chuva_fraca'] = (df_enhanced['precipitacao_mm'] > 0) & (df_enhanced['precipitacao_mm'] <= 2.5)
            ...
            if 'precipitacao_mm' in df_enhanced.columns:
                for hours in [3, 6, 12, 24]:
                    df_enhanced[f'precipitacao_sum_{hours}h'] = (
                        df_enhanced['precipitacao_mm'].rolling(window=hours, min_periods=1).sum()
                    )
                    df_enhanced[f'precipitacao_max_{hours}h'] = (
                        df_enhanced['precipitacao_mm'].rolling(window=hours, min_periods=1).max()
                    )
            ...
            if 'temperatura_c' in df_enhanced.columns:
                for hours in [6, 12, 24]:
                    df_enhanced[f'temperatura_mean_{hours}h'] = (
                        df_enhanced['temperatura_c'].rolling(window=hours, min_periods=1).mean()
                    )
                    df_enhanced[f'temperatura_std_{hours}h'] = (
                        df_enhanced['temperatura_c'].rolling(window=hours, min_periods=1).std()
                    )
            
            # Tendência de pressão
            if 'pressao_mb' in df_enhanced.columns:
                df_enhanced['pressao_trend_3h'] = (
                    df_enhanced['pressao_mb'].diff().rolling(window=3, min_periods=1).mean()
                )
                df_enhanced['pressao_trend_6h'] = (
                    df_enhanced['pressao_mb'].diff().rolling(window=6, min_periods=1).mean()
                )
    ...
    def unify_datasets(self, data_files: Dict[str, Dict]) -> pd.DataFrame:
        ...
        for file_key, file_data in data_files.items():
            ...
            if 'timestamp' in df_processed.columns:
                ...
                coverage = valid_rows / df_processed.shape[0] if df_processed.shape[0] > 0 else 0
        ...
        for i, df in enumerate(unified_data):
            ...
            if i == 0:
                ...
                if not df_clean.columns.equals(expected_columns):
                    logger.warning(f"⚠️ Dataset {i} tem colunas diferentes, alinhando...")
        ...
        if 'timestamp' in unified_df.columns:
            ...
            if pd.api.types.is_datetime64_any_dtype(unified_df['timestamp']):
                ...
                unified_df['timestamp'] = pd.to_datetime(unified_df['timestamp']).dt.tz_localize(None)
            ...
            if removed_duplicates > 0:
                logger.warning(f"⚠️ Removidas {removed_duplicates} linhas com timestamps duplicados")
        
        logger.info(f"✅ Dataset unificado criado: {unified_df.shape}")
        logger.info(f"📅 Período: {unified_df['timestamp'].min()} → {unified_df['timestamp'].max()}")
    ...
    def generate_preprocessing_report(self, original_data: Dict[str, Dict], 
        ...
        report = {
            ...
            'total_original_rows': sum(data['data'].shape[0] for data in original_data.values()),
    ...
    def save_processed_data(self, processed_data: pd.DataFrame, splits: Dict[str, pd.DataFrame], 
        ...
        numeric_columns = ['precipitacao_mm', 'pressao_mb', 'temperatura_c', 'umidade_relativa', 
                          'vento_velocidade_ms', 'vento_direcao_graus', 'radiacao_global_kjm2',
                          'temperatura_orvalho_c', 'vento_rajada_ms']
        ...
        for col in processed_data.columns:
            if col not in ['timestamp', 'estacao', 'data'] and processed_data[col].dtype == 'object':
        ...
        for split_name, split_df in splits.items():
            ...
            for col in split_df.columns:
                if col not in ['timestamp', 'estacao', 'data'] and split_df[col].dtype == 'object':
                    # Tentar converter para numérico
                    ...
                    if numeric_series.notna().sum() > len(split_df) * 0.5:
                        ...
                        logger.info(f"✅ Coluna {col} convertida para numérico no split {split_name}")
        ...
        processed_data.head(10000).to_csv(csv_path, index=False)  # Apenas primeiras 10k linhas
        ...
        logger.info(f"  - Período: {processed_data['timestamp'].min()} → {processed_data['timestamp'].max()}")
        logger.info(f"  - Tamanho em memória: {processed_data.memory_usage(deep=True).sum() / 1024**2:.1f} MB")
