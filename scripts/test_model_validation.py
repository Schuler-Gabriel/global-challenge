...
def create_synthetic_data(n_samples: int = 5000) -> pd.DataFrame:
    ...
    data = {
        ...
        'temperatura_c': 20 + 10 * np.sin(np.arange(n_samples) * 2 * np.pi / (24 * 365)) + np.random.normal(0, 2, n_samples),
    ...
    data['temperatura_max_c'] = data['temperatura_c'] + np.random.uniform(0, 3, n_samples)
    data['temperatura_min_c'] = data['temperatura_c'] - np.random.uniform(0, 3, n_samples)
    data['umidade_max'] = np.minimum(data['umidade_relativa'] + np.random.uniform(0, 10, n_samples), 100)
    data['umidade_min'] = np.maximum(data['umidade_relativa'] - np.random.uniform(0, 10, n_samples), 0)
    data['ponto_orvalho_max_c'] = data['ponto_orvalho_c'] + np.random.uniform(0, 2, n_samples)
    data['ponto_orvalho_min_c'] = data['ponto_orvalho_c'] - np.random.uniform(0, 2, n_samples)
...
def main():
    ...
    if passed == total:
        print("✅ Todos os testes passaram! Implementação da validação está funcionando.")
    else:
