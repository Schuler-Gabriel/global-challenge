...
class WeatherDataValidator:
            ...
            if 'Data' in df.columns and 'Hora UTC' in df.columns:
                ...
                try:
                    ...
                    try:
                        ...
                        df['datetime'] = pd.to_datetime(
                            df['Data'] + ' ' + df['Hora UTC'].astype(str).str.replace(' UTC', ''),
                            errors='coerce'
                        )
                ...
                if len(df) == 0:
                    logger.warning(f"Nenhum datetime válido encontrado em {filepath.name}")
                    return None
                
                # Verifica se perdeu muitos registros
                if len(df) < initial_count * 0.5:
                    logger.warning(f"Muitos registros com datetime inválido em {filepath.name}: {initial_count - len(df)} perdidos")
    ...
    def validate_range(self, series: pd.Series, variable_name: str) -> ValidationResult:
        ...
        if len(non_missing) > 0:
            ...
            outlier_percentage = (outlier_count / len(non_missing)) * 100 if len(non_missing) > 0 else 0
        else:
            out_of_range = 0
            valid_records = 0
            outlier_count = 0
        ...
        if missing_records > 0:
            issues.append(f"{missing_records} registros faltantes ({missing_records/total_records*100:.1f}%)")
        if out_of_range > 0:
            issues.append(f"{out_of_range} valores fora do range [{min_val}, {max_val}]")
        if outlier_count > 0:
            ...
            for gap_start in large_gaps.index:
                ...
                try:
                    gaps.append({
                        ...
                        'missing_records': int(gap_duration.total_seconds() / 3600) - 1
                    })
    ...
    def detect_anomaly_periods(self, df: pd.DataFrame, variable: str = 'precipitation') -> List[Dict[str, Any]]:
        """
    ...
    def calculate_quality_metrics(self, validation_results: List[ValidationResult], 
                                df: pd.DataFrame) -> QualityMetrics:
        """
        ...
        validity = (total_valid / total_non_missing * 100) if total_non_missing > 0 else 0
        
        # Consistency: baseado em gaps temporais
        consistency = 0.0
        try:
            if not df.empty and isinstance(df.index, pd.DatetimeIndex) and len(df.index) > 1:
                ...
                if not pd.isna(start_time) and not pd.isna(end_time):
                    ...
                    consistency = min(100.0, (actual_records / expected_records * 100)) if expected_records > 0 else 0
        except Exception as e:
        ...
        accuracy = ((total_non_missing - total_outliers) / total_non_missing * 100) if total_non_missing > 0 else 0
        
        # Score geral (média ponderada) - limita a 100
        overall_score = min(100.0, (completeness * 0.3 + validity * 0.3 + consistency * 0.2 + accuracy * 0.2))
    ...
    def calculate_summary_statistics(self, df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
        """
    ...
    def validate_file(self, filepath: Path) -> DataValidationReport:
        ...
        if 'PRECIPITAÇÃO TOTAL, HORÁRIO (mm)' in df.columns:
            precip_anomalies = self.detect_anomaly_periods(
                df.rename(columns={'PRECIPITAÇÃO TOTAL, HORÁRIO (mm)': 'precipitation'}),
                'precipitation'
            )
        ...
        try:
            date_range = (
                df.index.min().strftime('%Y-%m-%d %H:%M') if not df.empty and not pd.isna(df.index.min()) else "",
                df.index.max().strftime('%Y-%m-%d %H:%M') if not df.empty and not pd.isna(df.index.max()) else ""
            )
        except (AttributeError, ValueError) as e:
        ...
        logger.info(f"Validação concluída. Score de qualidade: {quality_metrics.overall_score:.1f}")
        return report
    ...
    def print_validation_summary(self, report: DataValidationReport) -> None:
        ...
        for result in report.variable_results:
            status = "✅" if result.validation_rate > 90 else "⚠️" if result.validation_rate > 70 else "❌"
            print(f"   {status} {result.variable_name}: {result.validation_rate:.1f}% válidos")
            if result.issues:
        ...
        if report.temporal_gaps:
            ...
            for gap in report.temporal_gaps[:5]:  # Mostra apenas os 5 primeiros
                print(f"   {gap['start_time']} → {gap['end_time']} ({gap['duration_hours']:.1f}h)")
        ...
        if report.anomaly_periods:
            ...
            for anomaly in report.anomaly_periods:
                if anomaly['type'] == 'extreme_precipitation':
                    print(f"   Chuva extrema: {anomaly['start_time']} ({anomaly['duration_hours']}h, {anomaly['max_value']:.1f}mm/h)")
        
        print("="*70)


def validate_directory(input_path: str, output_dir: str = "data/validation") -> List[DataValidationReport]:
    """
    ...
    # Se é um diretório, busca todos os CSVs
        ...
        logger.error(f"Caminho inválido (deve ser diretório ou arquivo CSV): {input_path}")
        return []
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validação de dados meteorológicos INMET")
    parser.add_argument("input_path", help="Diretório com arquivos CSV ou arquivo CSV específico para validar")
    ...
    try:
        ...
        if reports:
            avg_quality = sum(r.quality_metrics.overall_score for r in reports) / len(reports)
            print(f"\n🎯 RESUMO GERAL:")
