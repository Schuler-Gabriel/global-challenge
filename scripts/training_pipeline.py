...
class TemporalDataSplitter:
    ...
    def create_temporal_splits(self, data: pd.DataFrame) -> Generator[Tuple[pd.DataFrame, pd.DataFrame], None, None]:
        """
        ...
        while current_date + pd.DateOffset(months=self.validation_months) <= end_date and fold_count < self.max_folds:
            # Definir períodos
            ...
            if len(train_split) > 0 and len(val_split) > 0:
                logger.info(f"Fold {fold_count + 1}: Train até {train_end.strftime('%Y-%m-%d')}, "
...
class MeteorologicalMetrics:
    ...
    @staticmethod
    def calculate_mae_by_intensity(y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        for intensity, threshold in thresholds.items():
            if intensity == 'light':
                ...
                mask = (y_true >= thresholds['moderate']) & (y_true < thresholds['heavy'])
            elif intensity == 'heavy':
                mask = (y_true >= thresholds['heavy']) & (y_true < thresholds['very_heavy'])
    ...
    @staticmethod
    def calculate_skill_score(y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        skill_score = (hits - hits_random) / (hits + misses + false_alarms - hits_random)
        return skill_score
    
    @staticmethod
    def calculate_precipitation_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        """
        ...
        for name, threshold in RAIN_THRESHOLDS.items():
            if name != 'very_heavy':  # Evitar threshold muito alto
                skill = MeteorologicalMetrics.calculate_skill_score(y_true, y_pred, threshold)
...
class LSTMModelBuilder:
    ...
    @staticmethod
    def build_model(sequence_length: int, features_count: int, 
        ...
        if len(lstm_units) == 1:
            model.add(LSTM(lstm_units[0], input_shape=(sequence_length, features_count)))
        else:
...
class TrainingPipeline:
    ...
    def run_temporal_cross_validation(self, max_folds: int = 5) -> Dict[str, Any]:
        ...
        for fold_idx, (train_data, val_data) in enumerate(self.data_splitter.create_temporal_splits(data)):
            ...
            try:
                ...
                X_train, y_train = self.prepare_sequences(
                    train_data, METEOROLOGICAL_FEATURES, 'precipitacao_mm', 
                    model_config['sequence_length']
                )
                X_val, y_val = self.prepare_sequences(
                    val_data, METEOROLOGICAL_FEATURES, 'precipitacao_mm',
                    model_config['sequence_length']
                )
                
                if len(X_train) == 0 or len(X_val) == 0:
                    logger.warning(f"Fold {fold_idx + 1} sem dados suficientes, pulando...")
                    continue
                ...
                logger.info(f"Fold {fold_idx + 1} - MAE: {metrics['mae']:.3f}, "
        ...
        if fold_results:
            ...
            # Salvar resultados
            results_file = MODELS_PATH / 'temporal_validation' / f'cv_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            with open(results_file, 'w') as f:
                json.dump(final_results, f, indent=2, default=str)
            
            logger.info(f"Validação cruzada temporal concluída. Resultados salvos em {results_file}")
    ...
    def _calculate_cv_statistics(self, fold_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for metric in numeric_metrics:
            values = [fold[metric] for fold in fold_results if metric in fold and fold[metric] is not None]
            if values:
        ...
        stats['overall_success'] = stats['meets_accuracy_target'] and stats['meets_mae_target']
    ...
    def run_hyperparameter_optimization(self, max_trials: int = 20) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Iniciando otimização de hiperparâmetros com {max_trials} trials...")
        ...
        # Salvar resultados
        results_file = MODELS_PATH / 'hyperopt' / f'hyperopt_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        with open(results_file, 'w') as f:
    ...
    def _train_with_params(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Pipeline de Treinamento e Validação - Fase 3.2')
    parser.add_argument('--mode', choices=['temporal-cv', 'hyperopt', 'full-pipeline'], 
                       default='temporal-cv', help='Modo de execução')
    ...
    try:
        if args.mode == 'temporal-cv':
            ...
            if results:
                ...
                logger.info(f"Accuracy média: {results.get('accuracy_mean', 0):.3f} ± {results.get('accuracy_std', 0):.3f}")
                logger.info(f"MAE médio: {results.get('mae_mean', 0):.3f} ± {results.get('mae_std', 0):.3f}")
                logger.info(f"RMSE médio: {results.get('rmse_mean', 0):.3f} ± {results.get('rmse_std', 0):.3f}")
                logger.info(f"Accuracy >= 75%: {'✅' if results.get('meets_accuracy_target', False) else '❌'}")
                logger.info(f"MAE <= 2.0: {'✅' if results.get('meets_mae_target', False) else '❌'}")
        
        elif args.mode == 'hyperopt':
            results = pipeline.run_hyperparameter_optimization(max_trials=args.max_trials)
            ...
            cv_results = pipeline.run_temporal_cross_validation(max_folds=args.max_folds)
            
            # 2. Otimização de hiperparâmetros
            logger.info("Etapa 2: Otimização de hiperparâmetros")
            hyperopt_results = pipeline.run_hyperparameter_optimization(max_trials=args.max_trials)
            ...
            results_file = MODELS_PATH / f'full_pipeline_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            with open(results_file, 'w') as f:
                json.dump(final_results, f, indent=2, default=str)
            
            logger.info(f"Pipeline completo finalizado. Resultados salvos em {results_file}")
