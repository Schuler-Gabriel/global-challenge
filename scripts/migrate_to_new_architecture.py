...
class ArchitectureMigrator:
    ...
    def create_ml_platform_structure(self):
        ...
        def create_structure(base, structure):
            ...
            base.mkdir(parents=True, exist_ok=True)  # Criar diretório base primeiro
    ...
    def create_config_files(self, ml_platform, api_backend):
        ...
        try:
            ...
            # Fallback para formato manual se toml não estiver disponível
            import json
            
            with open(ml_platform / "pyproject.json", "w") as f:
                json.dump(ml_pyproject, f, indent=2)
            print("✅ ML Platform pyproject.json criado (toml não disponível)")
            
            with open(api_backend / "pyproject.json", "w") as f:
                json.dump(api_pyproject, f, indent=2)
            print("✅ API Backend pyproject.json criado (toml não disponível)")
