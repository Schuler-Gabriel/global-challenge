...
class StandardizationMetrics:
    """Métricas de padronização dos dados."""
    mean: float
    std: float
    min_value: float
    max_value: float
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "StandardizationMetrics":
...
class StandardizedDataset:
    """Dataset padronizado com métricas."""
    data: pd.DataFrame
    metrics: Dict[str, StandardizationMetrics]
    original_columns: List[str]
    processed_at: datetime
    total_rows: int
    
    def get_feature_metrics(self, feature_name: str) -> Optional[StandardizationMetrics]:
        """Obtém métricas de uma feature específica."""
        return self.metrics.get(feature_name)
    
    def get_standardized_column(self, column_name: str) -> Optional[pd.Series]:
        """Obtém coluna padronizada."""
        if column_name in self.data.columns:
            return self.data[column_name]
        return None
    
    def to_summary(self) -> Dict[str, Any]:
        """Retorna resumo do dataset padronizado."""
        return {
            "processed_at": self.processed_at.isoformat(),
            "total_rows": self.total_rows,
            "total_features": len(self.original_columns),
            "standardized_features": list(self.metrics.keys()),
