...
class WeatherData:
    ...
    def _validate_ranges(self):
        ...
        # Validar temperatura
        if not (valid_ranges['temperature'][0] <= self.temperature <= valid_ranges['temperature'][1]):
        ...
        # Validar pressão
        if not (valid_ranges['pressure'][0] <= self.pressure <= valid_ranges['pressure'][1]):
        ...
        if not (0 <= self.wind_speed <= valid_ranges['wind_speed'][1]):
            raise ValueError(f"Velocidade do vento fora do range válido: {self.wind_speed}")
    ...
    def is_extreme_weather(self) -> bool:
        ...
        """
        return (
            self.precipitation > 50.0 or
            self.temperature > 40.0 or
            self.temperature < 0.0 or
            self.wind_speed > 30.0 or
            self.humidity < 10.0
        )
...
class ModelMetrics:
    ...
    def meets_all_criteria(self) -> bool:
        ...
        """
        return (
            self.meets_mae_criteria() and
            self.meets_rmse_criteria() and
