...
from .entities import WeatherData, Forecast, ModelMetrics, PrecipitationLevel, WeatherCondition
...
class ForecastService:
    ...
    def should_generate_alert(self, forecast: Forecast, river_level: Optional[float] = None) -> bool:
        """
        ...
        if river_level is not None:
            ...
            if (forecast.get_precipitation_level() == PrecipitationLevel.LIGHT and 
                river_level > 2.5):
                return True
        
        return False
    
    def calculate_risk_score(self, forecast: Forecast, river_level: Optional[float] = None) -> float:
        """
...
class WeatherAnalysisService:
    ...
    def detect_patterns(self, weather_data: List[WeatherData]) -> Dict[str, Any]:
        ...
        patterns = {
            ...
            'avg_temperature': sum(data.temperature for data in weather_data) / len(weather_data),
            'avg_humidity': sum(data.humidity for data in weather_data) / len(weather_data),
            'avg_wind_speed': sum(data.wind_speed for data in weather_data) / len(weather_data),
            'extreme_events': sum(1 for data in weather_data if data.is_extreme_weather()),
        ...
        for i, data in enumerate(weather_data):
            ...
            if i > 0:
                ...
                if pressure_change > 20:
                    anomaly_info['anomalies'].append(f'pressure_change_{pressure_change:.1f}mB')
    ...
    def calculate_meteorological_indices(self, weather_data: List[WeatherData]) -> Dict[str, float]:
        """
...
class ModelValidationService:
    ...
    def compare_models(self, current_metrics: ModelMetrics, new_metrics: ModelMetrics) -> Dict[str, Any]:
        """
        ...
        improvements_count = sum(
            1 for metric in comparison['improvements'].values()
            if metric['better']
        )
    ...
    def recommend_model_update(self, comparison: Dict[str, Any], threshold: float = 0.05) -> Dict[str, Any]:
        """
        ...
        if improvements['mae']['better'] and abs(improvements['mae']['improvement']) > threshold:
            significant_improvements += 1
            recommendation['reasons'].append(f"MAE improvement: {improvements['mae']['improvement']:.3f}")
        
        if improvements['rmse']['better'] and abs(improvements['rmse']['improvement']) > threshold:
            significant_improvements += 1
            recommendation['reasons'].append(f"RMSE improvement: {improvements['rmse']['improvement']:.3f}")
        
        if improvements['accuracy']['better'] and abs(improvements['accuracy']['improvement']) > threshold:
            significant_improvements += 1
            recommendation['reasons'].append(f"Accuracy improvement: {improvements['accuracy']['improvement']:.3f}")
