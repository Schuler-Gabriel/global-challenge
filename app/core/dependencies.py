...
from .exceptions import APIConnectionError, CacheConnectionError, ConfigurationError
...
async def get_http_client() -> Generator[httpx.AsyncClient, None, None]:
    ...
    timeout = httpx.Timeout(
        connect=5.0, read=float(settings.api_timeout), write=5.0, pool=10.0
    )
...
async def get_rate_limiter(cache: CacheManager = Depends(get_cache)) -> RateLimiter:
    """
...
async def check_rate_limit(
    ...
    if not await rate_limiter.check_rate_limit(
        identifier=client_ip, limit=settings.rate_limit_per_minute, window=60
    ):
        raise HTTPException(
...
class HealthChecker:
    ...
    async def check_external_apis_health(
        ...
        # Check Guaíba API
        try:
            response = await http_client.get(self.settings.guaiba_api_url, timeout=5.0)
            results["guaiba_api"] = {
            ...
            results["guaiba_api"] = {"status": "error", "error": str(e)}

        # Check CPTEC API
        try:
            response = await http_client.get(self.settings.cptec_api_url, timeout=5.0)
            results["cptec_api"] = {
                "status": "healthy" if response.status_code == 200 else "unhealthy",
                "status_code": response.status_code,
                "response_time": response.elapsed.total_seconds(),
            ...
            results["cptec_api"] = {"status": "error", "error": str(e)}
