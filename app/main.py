...
from app.core.logging import setup_logging, get_structured_logger, get_request_context_logger
...
async def lifespan(app: FastAPI):
    ...
    """
    # Startup
    logger.info("Starting Flood Alert System API", extra={
...
if settings.is_production:
    app.add_middleware(
        ...
        allowed_hosts=["*.yourdomain.com", "yourdomain.com"]  # TODO: Configure actual domains
    )
...
async def application_exception_handler(request: Request, exc: BaseApplicationException):
    ...
    status_code = ERROR_STATUS_MAPPING.get(type(exc), status.HTTP_500_INTERNAL_SERVER_ERROR)
...
async def detailed_health_check():
    ...
    all_healthy = (
        ...
        all(api.get("status") == "healthy" for api in apis_health.values()) and
