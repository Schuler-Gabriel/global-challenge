...
def load_processed_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    logger.info(
        f"Dados carregados - Train: {len(train_data)}, Val: {len(val_data)}, Test: {len(test_data)}"
    )
...
def create_lstm_model(
    ...
    for i, units in enumerate(lstm_units[1:], 2):
        ...
        model.add(LSTM(units, return_sequences=return_sequences, name=f"lstm_{i}"))
...
def train_single_model(
    architecture: str, config_overrides: Dict[str, Any] = None, experiment: bool = False
) -> Dict[str, Any]:
    """
    ...
    if architecture not in ARCHITECTURES:
        raise ValueError(
            f"Arquitetura '{architecture}' não encontrada. Disponíveis: {list(ARCHITECTURES.keys())}"
        )
    ...
    logger.info(
        f"Sequências criadas - Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}"
    )
    ...
    callbacks = [
        ...
        TensorBoard(log_dir=str(tensorboard_dir), histogram_freq=1, write_graph=True),
    ]
...
def run_grid_search(architectures: List[str] = None, max_combinations: int = 10):
    ...
    logger.info(
        f"Testando {len(param_combinations)} combinações para {len(architectures)} arquiteturas"
    )
...
def main():
    ...
    parser.add_argument("--epochs", type=int, help="Número de epochs (override)")

    parser.add_argument("--batch-size", type=int, help="Tamanho do batch (override)")

    parser.add_argument(
        "--learning-rate", "-lr", type=float, help="Taxa de aprendizado (override)"
    )
    ...
    parser.add_argument(
        "--grid-search", "-g", action="store_true", help="Executar grid search"
    )
    ...
    try:
        if args.grid_search:
            ...
            result = train_single_model(
                args.architecture, config_overrides, args.experiment
            )
            ...
            if success_count == total_criteria:
                ...
                logger.warning(
                    f"⚠️  Atendeu {success_count}/{total_criteria} critérios de sucesso"
                )
