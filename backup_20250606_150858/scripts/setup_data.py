...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("data_setup.log"), logging.StreamHandler()],
)
...
class INMETDataSetup:
    ...
    def validate_csv_structure(self, filepath: Path) -> Tuple[bool, Optional[str], int]:
        ...
        try:
            ...
            missing_cols = [
                col for col in expected_cols if col not in df_sample.columns
            ]
    ...
    def run_setup(self) -> DataQualityReport:
        ...
        missing_percentage = (
            (len(corrupted_files) / len(file_metadata_list) * 100)
    ...
    def save_metadata(
        self, report: DataQualityReport, consolidated: Dict[int, List[FileMetadata]]
    ) -> None:
        """
        ...
        consolidated_file = (
            self.METADATA_PATH / f"consolidated_by_year_{timestamp}.json"
        )
        consolidated_dict = {
