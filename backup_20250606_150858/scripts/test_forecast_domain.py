...
def test_weather_data_entity():
    ...
    print(
        f"   ✅ Nível de precipitação: {weather_data.get_precipitation_level().value}"
    )
...
def test_forecast_entity():
    ...
    print(
        f"   ✅ Atende critérios de performance? {forecast.meets_performance_criteria()}"
    )
...
def test_model_validation_service():
    ...
    print(
        f"   ✅ Recomendação: {recommendation['confidence']} confiança para atualizar"
    )
...
def main():
    ...
    try:
        ...
        print(
            "   • Services: ForecastService, WeatherAnalysisService, ModelValidationService ✅"
        )
